...
namespace HW1
{
    class MainClass
    {
        ...
        {
            Console.WriteLine("Most Buys");
            var mostBuys = (from per in people
                            join purchase in purchases on per.Id equals purchase.PersonId into peopleShopInfo
                            orderby peopleShopInfo.Select(x => x.Quantity).Sum() descending
                            select new { FirstName = per.FirstName, LastName = per.LastName }).FirstOrDefault();

            Console.WriteLine("FirstName: " + mostBuys.FirstName + " LastName: " + mostBuys.LastName);
        }
